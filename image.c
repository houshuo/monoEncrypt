...
{
	__android_log_print(ANDROID_LOG_INFO, "JNITag", "Detect Attach");
	...
	if (fd != NULL)
	{
		while (fgets(line, bufsize, fd))
		{
			if (strncmp(line, "TracerPid", 9) == 0)
			{
				...
				__android_log_print(ANDROID_LOG_INFO, "JNITag", "TracerPid: %d", statue);
				if (statue != 0)
				{
					fclose(fd);
...
void init_decrypt_dll() __attribute__((constructor));
...
void init_decrypt_dll()
{
	init_func("mono_image_open_from_data_with_name");
}

void init_func(const char* target_fun){
	funcInfo info;
	int i;
	...
	if(getTargetFuncInfo(base, target_fun, &info) == -1){
	  print_debug(target_fun);
	  return ;
	}
	
	unsigned int page_base = (base + info.st_value) / PAGE_SIZE * PAGE_SIZE;
	unsigned int len = base + info.st_value + info.st_size - page_base;
	__android_log_print(ANDROID_LOG_INFO, "JNITag", "mprotect from addr =  0x%x, size = 0x%x", (base + info.st_value) / PAGE_SIZE * PAGE_SIZE, len);
	if(mprotect((void *) ((base + info.st_value) / PAGE_SIZE * PAGE_SIZE), len, PROT_READ | PROT_EXEC | PROT_WRITE) != 0){
	...
	for(i=0;i< info.st_size - 1; i++){
		...
		__android_log_print(ANDROID_LOG_INFO, "JNITag", "decrypt: 0x%x", base + info.st_value + i);
		*addr = ~(*addr);
	}
	__android_log_print(ANDROID_LOG_INFO, "JNITag", "decrypt finish");

	if(mprotect((void *) ((base + info.st_value) / PAGE_SIZE * PAGE_SIZE), info.st_size, PROT_READ | PROT_EXEC) != 0){
		print_debug("mem privilege change failed");
	}
	__android_log_print(ANDROID_LOG_INFO, "JNITag", "unshell finish");
}